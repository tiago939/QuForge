...
def State(dits, dim=2, device='cpu'):
    ...
    if isinstance(dim, int):
        ...
        if len(dims) != num_qudits:
            raise ValueError("Length of dim list must equal the number of qudits in the input.")
    ...
    for i, s in enumerate(qudit_strs):
        ...
        if index >= current_dim:
            raise ValueError(f"State index {s} exceeds dimension {current_dim} for qudit {i}.")
        
        # Create a basis tensor.
        base = torch.zeros((current_dim, current_dim, 1), device=device, dtype=torch.complex64)
...
def measure(state=None, index=[0], shots=1, dim=2, wires=None):
    ...
    if isinstance(dim, int):
        ...
        if wires is None:
            ...
            raise ValueError("The wires parameter must match the length of the dim list.")
...
def project(state, index=[0], dim=2):
    ...
    if total_dim != state.shape[0]:
        raise ValueError("The product of the individual qudit dimensions must equal the state dimension.")
    ...
    for i in range(wires):
        if i in index:
            ...
            U = torch.kron(U, torch.eye(dims_list[i], device=state.device, dtype=state.dtype))
...
def exp_value(state, observable='Z', index=0, dim=2):
    ...
    for i in range(wires):
        if i in indices:
            ...
            full_operator = torch.kron(full_operator, torch.eye(dims_list[i], dtype=torch.complex64, device=state.device))
...
def show(state, dim=2, wires=1, tol=1e-12, use_floats=False, 
    ...
    Parameters:
        ...
        suppress_inner (bool): 
            ...
            (e.g. |\phi_1\rangle, |\phi_2\rangle, etc.) instead of showing its full expansion.
    ...
    if isinstance(dim, int):
        ...
        if wires != len(dims_list):
            raise ValueError("For multidimensional qudits, wires must equal the length of the dim list.")
    ...
    # If emphasis is requested, factor out the specified qudit.
        ...
        for idx, amp in enumerate(state):
            ...
            try:
                ...
                raise ValueError("emphasize_index is out of range for the number of qudits.")
